export const moreArticlesXHRRequest = (component) => {
  let xhttp = new XMLHttpRequest();
  let oldArticles = component.state.articles.slice(0);
  let newArticles = [];
  ...
  xhttp.onreadystatechange = function() {
    if (xhttp.readyState === 4 && xhttp.status === 200) {
      ...
      if (obj !== "") {
        ...
        let end = component.state.loadNumber * 10 > newArticlesJSON.length ?
          newArticlesJSON.length : component.state.loadNumber * 10;
        ...
        if (end >= newArticlesJSON.length && component.state.beyondBootStrap === true) {
